...
munsters.each do |_, value|
  counter += value["age"] if value["gender"] == "male"
end
...
arr2 = []
arr1.each do |hash|
  arr2 << hash.transform_values { |value| value += 1}
end
...
new_hsh = hsh.map do |key, value|
  if value[:type] == "fruit"
    value[:colors].map { |cols| cols.capitalize}
  else
    value[:size].upcase
  end
end
...
#  p generate_uuid
 ...
 def generate_uuid()
  ...
  e = ""
  8.times do 
    a.prepend(HEX_CODE.sample)
  end
  4.times do
    b.prepend(HEX_CODE.sample)
    c.prepend(HEX_CODE.sample)
    d.prepend(HEX_CODE.sample)
  end
  12.times do
    e.prepend(HEX_CODE.sample)
  end
  "#{a}#{b}#{c}#{d}#{e}"
end
